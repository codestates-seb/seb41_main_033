...
const ProfileEdit = () => {
	...
	if (user) {
		return (
			<PostPatch image={user.profileImage} nickname={nickname} identifier={user.identifier} button1="수정완료" button2="탈퇴하기" link="/quit" handleSubmit={handlePatch}>
				<ContentWrap>
					<NicknameWrap>
						<label htmlFor="nickname">닉네임 수정</label>
						<InputWrap type="text" name="nickname" value={user.nickname || ""} onChange={handleNickname} />
					</NicknameWrap>
					<ProfileWrap>
						...
						<div className="custom_input_wrap">
							<div className="custom_input">
								{fileName}
								<input type="file" onChange={(e) => handleOnChange(e)} id="selectImg" />
							</div>
							...
							{gameList.games.map((game) => (
								<div className="game" key={game.id}>
									<input
										...
										defaultChecked={user.games.filter((games) => games.korTitle === game.title).length > 0 ? true : null}
									/>
					...
					<BioWrap>
						...
						<textarea id="bio" value={user.introduction || ""} placeholder="내용을 입력하세요" onChange={handleBio} />
					</BioWrap>
				</ContentWrap>
			</PostPatch>
		);
